...
def log_location_history(conn, employee_name, lat, lng):
    ...
    try:
        existing = conn.read(worksheet="LocationHistory", usecols=list(range(8)), ttl=5)
...
def get_current_location():
    result = st.session_state.get('current_location', {})
    if not result:
        result = st.experimental_get_query_params().get('location', {})
    return result.get("latitude"), result.get("longitude")

def check_existing_attendance(employee_name, check_type="checkin"):
    try:
        ...
        if check_type == "checkin":
            ...
            if not existing_records.empty:
                ...
                return existing_records['Check-out Time'].notna().any()
            return False
...
def record_attendance(conn, employee_name, status, check_type="checkin", location_link="", leave_reason=""):
    try:
        employee_code = Person[Person['Employee Name'] == employee_name]['Employee Code'].values[0]
        designation = Person[Person['Employee Name'] == employee_name]['Designation'].values[0]
        current_date = get_ist_time().strftime("%d-%m-%Y")
        ...
        attendance_id = f"ATT-{current_date.replace('-','')}-{str(uuid.uuid4())[:4].upper()}"
        ...
        if check_type == "checkin":
            ...
            attendance_data = existing_record.to_dict()
            attendance_data["Check-out Time"] = current_time
            attendance_data["Check-out Date Time"] = current_datetime
        
        attendance_df = pd.DataFrame([attendance_data])
        
        if check_type == "checkin":
            # For check-in, we always create a new record
            existing_data = conn.read(worksheet="Attendance", ttl=5)
            existing_data = existing_data.dropna(how="all")
            updated_data = pd.concat([existing_data, attendance_df], ignore_index=True)
        else:
            # For check-out, we update the existing record
            existing_data = conn.read(worksheet="Attendance", ttl=5)
            existing_data = existing_data.dropna(how="all")
            ...
            existing_data.loc[mask, 'Check-out Date Time'] = current_datetime
...
def attendance_page():
    ...
    with tab1:
        ...
        if check_existing_attendance(selected_employee, "checkin"):
            ...
            if status in ["Present", "Half Day"]:
                ...
                if lat and lng:
                    gmaps_link = f"https://maps.google.com/?q={lat},{lng}"
                    st.success(f"Location captured: [View on Google Maps]({gmaps_link})")
                else:
                    gmaps_link = ""
                    st.info("Could not retrieve location automatically. Please ensure location access is enabled.")
                
                if st.button("Check-in", key="checkin_button"):
                    if status in ["Present", "Half Day"] and not gmaps_link:
                        st.error("Location verification is required for check-in")
                    else:
    ...
    with tab2:
        ...
        if not check_existing_attendance(selected_employee, "checkin"):
            st.warning("Please check-in first before checking out.")
        elif check_existing_attendance(selected_employee, "checkout"):
            st.warning("You have already checked out today.")
        else:
            st.info("You can check out for the day.")
            ...
            if st.button("Check-out", key="checkout_button"):
                with st.spinner("Recording check-out..."):
                    success, error = record_attendance(
                        conn,
                        selected_employee,
                        "",  # Status is not needed for checkout
                        "checkout",
                        gmaps_link
                    )
                    if error:
                        st.error(f"Failed to record check-out: {error}")
                    else:
...
def main():
    ...
    if not st.session_state.authenticated:
        ...
        page = st.sidebar.radio("Go to", ["Attendance", "Support Ticket", "Resources"])
